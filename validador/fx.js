...
{
	var form2 		= $("form[name='form_cotiza']");
	var cotizame	= form2.find("[name='enviar']");

	cotizame.on("click", function(e)
	{
		
		if(form2.attr("data-status") == "standby")
		{
			form2.attr("data-status","working");			
		
			$(".alert").remove();
		
			var str 	= "nombre,apellido,rut,email";
			...
			for(var v = 0; v < arr.length; v++)
		    {
		    	var item 	= form2.find("[name='" + arr[v] + "']");
		    	var tipo 	= item.attr('data-valid');
		    	var mensaje	= item.attr('data-message');

				if(validador(form2,arr[v],tipo,mensaje,'width: 100%; margin-top:-10px;padding: 9px;') == 1)
			    {
			        err     = 1;
			        form2.attr("data-status","standby");
			        return false;
			    }
			}
...
function cambia_info()
{
	...
	var modelo = document.getElementById("modelo_select");
    modelo.name = "of_" + oficina;
